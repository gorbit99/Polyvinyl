...
struct VQFParams {
	 ...
	 * measurements and while large values of
...
struct VQFState {
	 ...
	 * magnetometer sample and the state
	 * \f$\delta\f$ (in rad).
	 */
...
public:
	 ...
	 * with the same frequency, create the class like this:
	 * \rst
	 * .. code-block:: c++
	 *
	...
	/**
	 * @brief Performs filter update step for one sample (with magnetometer
	 * measurement).
	 * @param gyr gyroscope measurement in rad/s
	 * @param acc accelerometer measurement in m/s²
	 * @param mag magnetometer measurement in arbitrary units
	 */
	void
	update(const vqf_real_t gyr[3], const vqf_real_t acc[3], const vqf_real_t mag[3]);

	/**
	 * @brief Performs batch update for multiple samples at once.
	 *
	 * In order to use this function, the input data must be available in an array in
	 * row-major order. A null pointer can be passed for mag in order to skip the
	 * magnetometer update. All data must have the same sampling rate.
	 *
	 * The output pointer arguments must be null pointers or point to sufficiently large
	 * data buffers.
	 *
	 * Example usage:
	 * \rst
	 ...
	 *
	 *     VQF vqf(0.01); // 0.01 s sampling time, i.e. 100 Hz
	 *     vqf.updateBatch(gyr, acc, mag, nullptr, quat9D, nullptr, nullptr, nullptr,
	 * nullptr, nullptr);
	 * \endrst
	 *
	 ...
	 * @param mag magnetometer measurement in arbitrary units (N*3 elements, can be a
	 * null pointer)
	 * @param N number of samples
	 * @param out6D output buffer for the 6D quaternion (N*4 elements, can be a null
	 * pointer)
	 * @param out9D output buffer for the 9D quaternion (N*4 elements, can be a null
	 * pointer)
	 * @param outDelta output buffer for heading difference angle (N elements, can be a
	 * null pointer)
	 * @param outBias output buffer for the gyroscope bias estimate (N*3 elements, can
	 * be a null pointer)
	 * @param outBiasSigma output buffer for the bias estimation uncertainty (N
	 * elements, can be a null pointer)
	 * @param outRest output buffer for the rest detection state (N elements, can be a
	 * null pointer)
	 * @param outMagDist output buffer for the magnetic disturbance state (N elements,
	 * can be a null pointer)
	 */
	void updateBatch(
		const vqf_real_t gyr[],
		const vqf_real_t acc[],
		const vqf_real_t mag[],
		size_t N,
		vqf_real_t out6D[],
		vqf_real_t out9D[],
		vqf_real_t outDelta[],
		vqf_real_t outBias[],
		vqf_real_t outBiasSigma[],
		bool outRest[],
		bool outMagDist[]
	);
